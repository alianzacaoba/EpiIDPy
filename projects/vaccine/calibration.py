...
class Calibration(object):
    ...
    def run(self) -> dict:
        ...
        try:
            ...
            x_L = list()
            y_L = list()
            x = np.random.triangular(0, 0.05, 1, size=initial_cases)
            for i in range(initial_cases):
                print('ITERATION:', i + 1, datetime.datetime.now())
                x_L.append(x[i])
                kwargs['beta'] = x[i]
            ...
            if theta[0] != 0:
                ...
                x_new = np.random.triangular(0, 0.05, 1)
...
results = calibration_model2.run()
for c in results:
    print(c, results[c])
end_time = datetime.datetime.now()
